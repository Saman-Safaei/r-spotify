...
function SigninModal({ show, onHide = () => {} }: props) {
  const { isError, error, isLoading, signin } = useSignin();

  const backgroundBg: string = isLoading
    ? 'bg-gradient-to-r from-slate-500 via-slate-700 to-slate-500 animate-bg'
    : 'bg-green-500 hover:bg-green-600';
  const buttonTxt: string = isLoading ? 'Please Wait ...' : 'Sign in';
  ...
  return (
    <Modal show={show} onHide={onHide} modalClass='py-6' backdropClass='px-4'>
      ...
      <h3 className='text-xl text-center font-bold text-gray-300 mb-6'>Sign in to your account to start listening.</h3>
      <Formik initialValues={defaultValues} validationSchema={validationSchema} onSubmit={signin}>
        <Form noValidate className='flex flex-col items-stretch gap-2'>
          <div>
            <div className='bg-neutral-700/50 focus-within:bg-neutral-700/70 rounded-md transition-colors duration-300'>
              <Field name='username' type='text' placeholder='Username' className='w-full px-3 py-3' />
            </div>
          ...
          <div>
            <div className='mb-2 bg-neutral-700/50 focus-within:bg-neutral-700/70 rounded-md transition-colors duration-300'>
              <Field name='password' type='password' placeholder='Password' className='w-full px-3 py-3' />
            </div>
            <p className='px-3 text-red-500 leading-7'>
          ...
          <div className='flex flex-col gap-2 items-stretch'>
            {isError && <p className='bg-red-500/20 px-3 py-3 rounded-md text-center'>{error?.message}</p>}
            <button
              type='submit'
              disabled={isLoading}
